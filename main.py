...
from db import Link, URLRequest, URLResponse
from datetime import datetime
...
async def shorten_url(
    ...
    request: URLRequest,
    db: AsyncSession = Depends(get_async_session)
    # Добавляем объект запроса для использования в теле функции
):
    # Получаем пользователя внутри функции
    try:
        # Используем объект запроса для получения пользователя
        user = await current_active_user(request_obj)
    except BaseException:
        user = None
    # Генерация alias для короткой ссылки
    ...
    user_id = user.id if user else None  # Если пользователя нет, user_id будет None
    ...
    new_link = Link(
        ...
        user_id=user_id  # Если пользователь не авторизован, user_id будет None
    )
...
async def search_link(original_url: str = Query(...), db: AsyncSession = Depends(get_async_session)):
    ...
    return {"short_code": link_data.short_code,
